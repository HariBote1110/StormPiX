...
export function emitAnimationLuaLzFrames(
  ...
  const decoder = `S=screen P="${palette.data}"d="${data}"o={}m=math.floor function V(n)return n-(n>95 and 53 or 48)end i=1 while i<=#d do local z=d:byte(i)if z==33 then local x=V(d:byte(i+1))*64+V(d:byte(i+2))+1 local n=V(d:byte(i+3))*64+V(d:byte(i+4))+3 for j=1,n do o[#o+1]=o[#o-x+1]end i=i+5 else local n=V(z)i=i+1 if n<12 then for j=1,n+1 do ${literalDecoder}end else local x=V(d:byte(i))*64+V(d:byte(i+1))+1 for j=1,n-9 do o[#o+1]=o[#o-x+1]end i=i+2 end end end F=S.drawRectF `;
