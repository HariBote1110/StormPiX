...
export function floorCoord(value: number): number { return Math.floor(value); }
...
function renderInternal(ops: readonly DrawOp[], width: number, height: number): RenderedWithMask {
  ...
  const fillRect = (x: number, y: number, w: number, h: number): void => {
    if (w <= 0 || h <= 0) return;
    const rangeX = clippedRange(x, x + w - 1, width);
    const rangeY = clippedRange(y, y + h - 1, height);
  ...
  const outlineRect = (x: number, y: number, w: number, h: number): void => {
    if (w <= 0 || h <= 0) return;
    for (let px = x; px < x + w; px += 1) { paintPixel(px, y); if (h > 1) paintPixel(px, y + h - 1); }
    for (let py = y + 1; py < y + h - 1; py += 1) { paintPixel(x, py); if (w > 1) paintPixel(x + w - 1, py); }
  };
  ...
  const drawLine = (x1: number, y1: number, x2: number, y2: number): void => {
    ...
    for (let step = 0; step <= steps; step += 1) {
      ...
      paintPixel(floorCoord(x1 + deltaX * ratio), floorCoord(y1 + deltaY * ratio), UNCERTAIN);
    }
  };
  ...
  const fillConvexPolygon = (points: readonly [number, number][]): void => {
    ...
    for (let y = minY; y <= maxY; y += 1) {
      ...
      const range = clippedRange(intersections[0] as number, intersections[intersections.length - 1] as number, width);
      if (!range) continue;
  ...
  const drawTriangle = (op: Extract<DrawOp, { type: 'triangle' | 'triangleF' }>): void => {
    ...
    } else fillConvexPolygon([[op.x1, op.y1], [op.x2, op.y2], [op.x3, op.y3]]);
  };
